<template>
	<div class="page-container">
		...
		<!-- 修改密碼卡片 -->
		<div class="card mb-4">
			<h2 class="section-title" style="font-size: 16px; margin-bottom: var(--space-5);">修改密碼</h2>
			...
			<form @submit.prevent="handleUpdatePassword">
				...
				<div class="form-footer">
					...
					<span v-if="passwordSuccessMessage" class="feedback feedback--success">{{ passwordSuccessMessage }}</span>
					<span v-if="passwordErrorMessage"   class="feedback feedback--error">{{ passwordErrorMessage }}</span>
				</div>
			</form>
		</div>
...
const handleUpdatePassword = async () => {
	if (newPassword.value !== confirmPassword.value) {
		passwordErrorMessage.value = '兩次輸入的密碼不一致'
		return
	}
	if (newPassword.value.length < 6) {
		passwordErrorMessage.value = '密碼長度必須至少為 6 碼'
		return
	}
	...
	try {
		// 調用 Supabase Auth API 更新當前使用者密碼
		...
		passwordSuccessMessage.value = '密碼已成功修改！'
		newPassword.value = ''
		confirmPassword.value = ''
		setTimeout(() => { passwordSuccessMessage.value = '' }, 3000)
	} catch (error: any) {
		passwordErrorMessage.value = error.message || '修改密碼失敗，請重試'
	} finally {
